...
const WASM_URL = `${BASE_URL}/wasm/hdh_security_bg.wasm`;
const PAGE_SIZE = 40;
...
function responseDataToBytes(data) {
  ...
  if (/^[A-Za-z0-9+/]+=*$/.test(compact) && compact.length % 4 === 0) {
    try {
      return base64DecodeBytes(compact);
    } catch (err) {}
  }

  const bytes = new Uint8Array(data.length);
  for (let i = 0; i < data.length; i += 1) bytes[i] = data.charCodeAt(i) & 255;
  return bytes;
}
...
async function loadSecurityWasmInner(cookieString) {
  ...
  ensureCrypto();
  const response = await Widget.http.get(WASM_URL, {
    base64Data: true,
    headers: buildHeaders(cookieString, { "Accept": "application/wasm,*/*" })
  });
  const wasmBytes = responseDataToBytes(response && response.data);

  if (!wasmBytes || wasmBytes.length < 1000) {
    throw new Error("HDHive 签名 WASM 下载失败或不是二进制数据");
  }
